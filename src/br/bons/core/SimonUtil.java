...
/**
 * Repositório de fórmulas - FWM - classe utilitária. Código original obtido a
 * partir de formulasFWM.h, por Daniel Augusto Ribeiro Chaves e Júlio Dantas de
 * Andrade.
 * 
...
public class SimonUtil {
	...
	/**
	 * Função Eficiencia = a*(1+b/c)
	 * 
	 * onde a=(aten^2)/((aten^2)+ (Casamento^2))
	 * b=4*exp(-aten*L)*(sin(casamento*L/2))^2 c=(1-exp(-aten*L))^2
	 * 
	 * 
	 * (aten²) ( 4.exp(-aten.L).sin²(Casamento*L/2) ) formula
	 * ----------------------.( 1 + ----------------------------------- )
	 * (aten²) + (Casamento²) ( (1-exp(-aten.L))² )
	 **/
	 ...
	 * 
	 * n ( (1-exp(-aten.L))² ) Formula:
	 * ---.(D²).(gama²).Pi.Pj.Pk.exp(-aten*L).(-------------------) 9 ( aten² )
	 * 
	 * alt+253= (²)
	 * 
	 * @param length
	...
	public static double getFWMPower(double length, double alfa, double fi, double fj, double fk, double Pi, double Pj,
		...
		double D = 3.0;
		// converte o coeficiente de atenuação de dB/km para m^-1
...
//		}
		...
		while (qtde < 10000000 && !(media < 2.5429 && media > 2.542)){
			...
			media = soma/n;
			qtde++;
			System.out.printf("média = %.4f; x = %.4f; a = %.4f\n", media, x, a);
		}
