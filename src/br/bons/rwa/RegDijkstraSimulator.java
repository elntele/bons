...
import static br.bons.core.Call.BIDIRECIONAL;
import static br.bons.rwa.Dijkstra.dijkstra_fnb;
...
import br.bons.core.BlockUtil;
import br.bons.core.Call;
import br.bons.core.CallList;
import br.bons.core.CallSchedulerUniform;
import br.bons.core.Fiber;
import br.bons.core.Link;
import br.bons.core.NetworkBP;
import br.bons.core.NetworkProfile;
import br.bons.core.Node;

public class RegDijkstraSimulator extends SimpleDijkstraSimulator {
	
	private boolean regRealizada = false;

	@Override
	public void simulate(NetworkProfile network) {
		int x = 0, y = 0, tempFiberAvailble = 0;
		int nLambdaMax = 0;
		int usarLambda = 0;
		...
		for (int i = 1; i <= numOfCalls; i++) {
			...
			if(usarLambda >= 0){
				...
				//senï¿½o, passe direto, iï¿½, bloqueie de acordo com status_loc, calculado anteriormente.
				if(regeneratorLocations_loc.size()>2){
					...
					if(usarLambda == BLOQ_WAVELENGTH){  //se bloquear por comprimento de onda, tente regenerar
						...
						//POR ENQUANTO, BLOQUEIE AS CHAMADAS Nï¿½O ESTABELECIDAS DEVIDO A BLOQ_WAVELENGTH.

		                //obs: AO sair deste trecho do cï¿½digo, fazer usarLambda_loc um nï¿½mero positivo qualquer;
			...
			if (usarLambda == BLOQ_WAVELENGTH) {
				...
				if (caminhoTotalmenteOptico){
		            ...
		            for(int j = 0; j < rotaUplink.size(); j++){	  //scanning all links in in found rote
		               tempFiberAvailble = rotaUplink.get(j).getAvailableFiber(usarLambda);  //ja ï¿½ implementaï¿½ï¿½o p multi-fibra
	...
	*******************************************************************/
	//====================================================================================//
	//=====================Novo cï¿½digo de regenerados para QoS============================//
	//==========Regenera no nï¿½ mais distante partindo do mais prï¿½ximo da origem===========//
	//====================================================================================//
	private int statusRotaReg_fnb(NetworkProfile network, int origem,
		...
		//seta informaï¿½ï¿½es a partir da primeira chamada do vetor
		...
		while(true){
			...
			if(testeQos_uplink){
				...
				if(testeQos_downlink){ //QoS downlink satisfeito?
					...
					if( tempDest_index < (regeneratorLocationsLoc.size() - 2) ){ // ainda hï¿½ reg. adiante? SIM
		               ...
		               ////////////////////////////////////////////////////////////
					...
					else{ //Ainda hï¿½ reg. adiante? NAO
						
						if( tempOrig_index == 0 ){
							
							//REGENERA O SINAL --- INï¿½CIO //
		                    //calcula a potencia de entrada
							somatorioPotencias = 0.0;
		                    ...
		                    // Decrementar a qtd de regeneradores livres no nï¿½;
							...
							//Nï¿½ regenerador analisado ï¿½ assumido como nï¿½ fonte intermediï¿½rio (NOVO PSEUDOCï¿½DIGO)
						...
						}//  chave do  else { //Ainda hï¿½ reg. adiante? NAO (ETAPA 2)
	...
	*******************************************************************/
	//====================================================================================//
	//=====================Novo cï¿½digo de regenerados para QoS============================//
	//Converte no nï¿½ mais distante partindo do mais prï¿½ximo da origem e tambï¿½m regenera por QoS//
	//====================================================================================//
	private int statusRotaConv_fnb(NetworkProfile network, int origem,
		...
		//seta informaï¿½oes apartir da primeira chamada do vetor
	   double tempodequeda_loc  =  listOfCalls.getListaChamadas().get(0).getFallTime();
	   double duracao_loc       =  listOfCalls.getListaChamadas().get(0).getDuration();
	   ...
	   //Antes de calcular a potï¿½ncia de entrada, deve fazer o FF da origem atï¿½ o primeiro nï¿½ com regenerador
	 ...
	 //=============INï¿½CIO BUSCA LAMBDA TRECHO A TRECHO===========================
	         ...
	         if(lambda_trecho_Atual_loc == BLOQ_WAVELENGTH){
	        	 ...
	        	 if(testeQoS_uplink_loc){
	        		 ...
	        		 if(testeQoS_downlink_loc){ //QoS satisfeito?
	        			 ...
	        			 if(tempDest_index < (regeneratorLocations.size()-2)){ //ainda hï¿½ reg. adiante? SIM
	                         ...
	                         //Chegando nesse trecho, a chamada serï¿½ sempre UNIDIRECIONAL_REGENERADA
	                         ...
	                         //Chegando nesse trecho, a chamada será sempre UNIDIRECIONAL
	        		 ...
	        		 if(!testeQoS_uplink_loc || !testeQoS_downlink_loc){
	        			 if((tempDest_index-tempOrig_index) == 1){
	        	           ...
	        	           //Decrementa a qtd de regeneradores livres no nï¿½;
	        	                 network.getNodes().get(regeneratorLocations.get(tempDest_index)).decNumFreeRegenerators();
	        	                 network.getNodes().get(regeneratorLocations.get(tempDest_index)).markRegenerators();
	...
	*******************************************************************/
	private int firstFit_fnb(Vector<Link> trechoEmAnaliseUplinkLoc,
		...
		for(int nLambda_loc=0; (nLambda_loc < nLambdaMax_par); nLambda_loc++) //procura para cada lambda
		{
			...
			// Procura em cada enlace o comprimento de onda disponï¿½vel de mesmo ï¿½ndice
		...
		if(lambdaEncontrado == -1){	//nao hï¿½ lambda disponivel
			return BLOQ_WAVELENGTH;
		}
		return 0;
	}
